...
public class BeatBox {
	...
	// Ezek a hangszerek nevei String tömbként, a grafikus felületen (egy-egy
	// sorban) megjelenő szövegcímkék számára
	...
	// Ezek a dob hangszerek, amik akkor szólalnak meg, ha a csatorna "9" a midi
	// esemény message paraméterében
	...
	public void buildGUI() {
		...
		JPanel background = new JPanel(bLayout); // A backgroundra kerül rá a hangszerek boxa, a középső checkbox grid
													// és
		// a jobb szélső button box.
		// Az "üres szegély" margót biztosít a panel széla és az összetevők között -
		// pusztán a vonzóbb látvány kedvéért
		...
		// A gombok box létrehozása és feltöltése gombokkal. A gombok közé rugalmas
		// függőleges elválasztást teszünk.
		...
		resetTempo.addActionListener(new MyTempoReseListener());
		...
		openButton.addActionListener(new MyOpenButtonListener());
		buttonBox.add(openButton);

		buttonBox.add(Box.createVerticalGlue());

		// A háttér panelre rátesszük balra a név boxot, középre a checkboxgridet, a
		// jobb szélre a gombokat
	...
	public void buildTrackAndStart(boolean start) {
		/*
		 * Létrehozunk egy 16 elemű tömböt, amelyben egy hangszer értékeit tároljuk a 16
		 * ütemre Ha egy hangszernek meg kell szólalnia egy adott ütemben, a tömb adott
		 * elemének értéke lesz a kulcs. Ha a hangszernek nem kell megszólalnia az
		 * említett ütemben, akkor nullát szúrunk be. A metódus a checkboxokból
		 * kiolvasott ki/be állapotoktól függően létrehozza a lejátszandó tracket.
		 * Ha a metódus paramétere true, akkor a lejátszás elindul, ha hamis, akkor nem indul el.
		 */
		int[] trackList = null;
		...
		for (int i = 0; i < 16; i++) { // Mind a 16 sorra (hangszerre) elvégezzük a beállítást
			...
			for (int j = 0; j < 16; j++) { // Az adott hangszer 16 ütemére elvégezzük a beállítást
				...
				System.out.print(jc.isSelected()+" ");
				// Be van kapcsolva a jelölőnégyzet erre az ütemre? Ha igen, a tömb megfelelő
				// rekeszébe (az
				// adot ütemeber jelképező rekeszbe) beszúrjuk a kulcsértéket (hangszert), ha
				// viszont nem
				...
				// Az adott hangszer mind a 16 üteméhez eseményeket hozunk létre és hozzáadjuk
				// azokat a sávhoz
			System.out.println("");
			makeTracks(trackList);
			track.add(makeEvent(176, 1, 127, 0, 16)); // vezérlőesemény hozzáadása
		} // külső for ciklus
			// Mindig gondoskodni kell róla, hogy legyen esemény a 16. ütemhez (a számozás
			// 0-tól 15-ig tart),
			// különben a BeatBox esetleg nem megy végig mind a 16 ütemen, mielőtt újrakezdi
			// a ciklust
		track.add(makeEvent(192, 9, 1, 0, 15));
		try {
			sequencer.setSequence(sequence);
			sequencer.setLoopCount(sequencer.LOOP_CONTINUOUSLY); // a ciklus ismétléseinek száma, folyamatos lejátszás
			if (start) sequencer.start();
			sequencer.setTempoInBPM(120);
		} catch (Exception e) {
			e.getStackTrace();
		}
	...
	// "Start" gomb figyelője, az aktuális checkbox beállítást kiolvasva lejátssza a
	// beállított "kottát"
	public class MyStartListener implements ActionListener {
		public void actionPerformed(ActionEvent a) {
			buildTrackAndStart(true);
		}
	...
	public class MyTempoReseListener implements ActionListener {
		public void actionPerformed (ActionEvent a) {
	...
	public class MyRandomMusicListener implements ActionListener {
		public void actionPerformed(ActionEvent a) {
			...
			int suruseg; // Mennyire sűrűn legyen töltve a kotta. Minél kisebb, annál több checkbox lesz
							// pipálva
	...
	public void refreshSpeedButtons() {
		...
		((JButton) (((Box) (((JPanel) theFrame.getContentPane().getComponent(0)) // background
		...
		/*
		 * JPanel c=((JPanel) theFrame.getContentPane().getComponent(0)); Box
		 * b=(Box)c.getComponent(0); ((JButton) (b.getComponent(7))).setText("HELLO");
		 */
	} // refreshSpeedButtons metódus

	public void makeTracks(int[] list) {
		/*
		 * Egyszerre egy hangszer számára hoz létre eseményeket, mind a 16 ütemre.
		 * Vagyis lehet egy int[] a basszusdobhoz és a tömb egyes rekeszeiben a hangszer
		 * kódja vagy nulla áll. Ha nulla, a hangszernek nem kell megszólalnia az adott
		 * ütemben, egyébként létrehozunk egy eseményt és hozzáadjuk a sávhoz.
		 */
		for (int i = 0; i < 16; i++) {// A hangszerhez tartozó ütemeken végigmegyünk
			...
			if (key != 0) { // Ha a hangszer kódja volt a tömbben
				...
				// A 9, mint csatorna a dob csatorna, amin belül más-más dob típusokat lehet
				// megszólaltatni
				// ami jelen esetben a key. Ha a 9-et 1-re cseréljük, akkor az instruments
				// tömbben lévő kódoknak
	...
	public class MySaveButtonListener implements ActionListener {
		public void actionPerformed(ActionEvent a) {
			...
			int returnVal;
			returnVal = fileOpen.showSaveDialog(theFrame);
			if (returnVal != JFileChooser.CANCEL_OPTION) {
				...
				try {
					...
					os.close();
					JOptionPane.showMessageDialog(theFrame, "A "+fileOpen.getSelectedFile()+" fájl mentése sikeres!", "Mentés",
							JOptionPane.INFORMATION_MESSAGE);
				} // try
				catch (Exception ex) {
					JOptionPane.showMessageDialog(theFrame, "File operation failed!", "Error",
	...
	public class MyOpenButtonListener implements ActionListener {
		public void actionPerformed(ActionEvent a) {
			sequencer.stop();
			JFileChooser fileSave = new JFileChooser();
			int returnVal;
			returnVal = fileSave.showOpenDialog(theFrame);
			if (returnVal != JFileChooser.CANCEL_OPTION) {
